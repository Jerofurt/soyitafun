...
function parseFormData(formData: FormData): ParseResult {
  ...
  return {
    ...
    data: {
      ...
      preco_base: parseNumber(formData.get('preco_base')),
...
function pgErrorMessage(error: { code?: string | null; message: string }): string {
  if (error.code === '23505') {
    return 'Já existe um serviço com este slug. Escolha outro.'
  }
...
export async function createServico(
  ...
  redirect(`/admin/servicos/${data.id}/editar?saved=1`)
}
...
export async function updateServico(
  ...
  if (orphanedPhotos.length > 0) {
    await supabase.storage.from('servicos').remove(orphanedPhotos)
  }

  revalidatePath('/admin/servicos')
  revalidatePath(`/admin/servicos/${id}/editar`)
  revalidatePath('/admin')
  redirect(`/admin/servicos/${id}/editar?saved=1`)
}
...
export async function deleteServico(id: string): Promise<void> {
  ...
  if (error) {
    ...
    redirect(`/admin/servicos/${id}/editar?${qs.toString()}`)
  }

  const { data: files } = await supabase.storage.from('servicos').list(id)
  if (files && files.length > 0) {
    const paths = files.map((f) => `${id}/${f.name}`)
    await supabase.storage.from('servicos').remove(paths)
  }
