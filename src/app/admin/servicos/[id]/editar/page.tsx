...
export default async function EditarServicoPage({
  ...
  const { data, error } = await supabase
    .from('servicos')
    .select(
      'id, slug, nome, tipo, descricao, fotos, preco_base, whatsapp, instagram, plano, status, data_inicio, data_vencimento, destaque',
    )
    .eq('id', id)
    .single()
  ...
  const initial = data as ServicoInitial
