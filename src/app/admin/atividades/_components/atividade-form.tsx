...
type Props =
  ...
  | { mode: 'edit'; initial: AtividadeInitial; saved?: boolean }

export function AtividadeForm(props: Props) {
  const { mode, saved } = props
  const initial = props.mode === 'edit' ? props.initial : undefined

  // Stable id for the photo folder prefix in the atividades bucket. Pre-
  // generated in create mode so uploads can land before the row exists.
  const [atividadeId] = useState(() => initial?.id ?? crypto.randomUUID())

  const action =
    mode === 'create'
      ? createAtividade
      : updateAtividade.bind(null, atividadeId)
        ...
        <Card>
          <CardBody className="p-12 space-y-12">
            <Section title="Informações básicas">
              <Field label="Nome" htmlFor="nome">
                <Input
                  ...
                  placeholder="Ex: Passeio de lancha pela Ilha Anchieta"
                />
              </Field>
              ...
              >
                <Input
                  ...
                  placeholder="passeio-lancha-ilha-anchieta"
                />
              </Field>
              ...
              <div className="grid grid-cols-2 gap-4">
                <Field label="Duração (horas)" htmlFor="duracao_horas">
                  <Input
                    ...
                    min="0"
                    step="0.5"
                    defaultValue={initial?.duracao_horas ?? ''}
                    placeholder="3"
                  />
                </Field>
                <Field label="Capacidade máxima" htmlFor="capacidade_max">
                  <Input
                    id="capacidade_max"
                    name="capacidade_max"
                    type="number"
                    min="1"
                    step="1"
                    defaultValue={initial?.capacidade_max ?? ''}
                    placeholder="8"
                  />
                </Field>
              </div>
            ...
            <Section title="Encontro e localização">
              <Field label="Ponto de encontro" htmlFor="ponto_encontro">
                <Input
                  id="ponto_encontro"
                  name="ponto_encontro"
                  defaultValue={initial?.ponto_encontro ?? ''}
                  placeholder="Ex: Quiosque do Pier, Praia de Itamambuca"
                />
              </Field>
            ...
            <Section title="Contato">
              <Field
                ...
                htmlFor="whatsapp_operador"
                hint="É o WhatsApp para onde a reserva chega — do operador, não da Cata."
              >
                <Input
                  id="whatsapp_operador"
                  name="whatsapp_operador"
                  type="tel"
                  defaultValue={initial?.whatsapp_operador ?? ''}
  ...
  return (
    <div className="mt-16 pt-8 border-t border-error/20">
      ...
      <p className="text-sm text-texto-secundario mt-2 mb-6 max-w-md leading-relaxed">
        Excluir esta atividade é permanente. As fotos no Storage também serão
        removidas.
      </p>
